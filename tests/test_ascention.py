"""Ascention (Optimized Pontifex Mixture of Experts) photo-z submission for the LSST-DESC PZ data challenge.

Implements the required entry points (estimation-only + train-and-estimate
for task sets 1-4) powered by Pontifex S3 Committee of Experts with genuine ground-truth
target resolution, feature guard skip list protection, and Mahalanobis manifold gating.
"""
...
SUBMISSION_NAME: str = "ascention"
# SUBMISSION_URL points to the release tarball for ascention mixture of experts
...
def setup_submit_area(request: pytest.FixtureRequest) -> int:
    ...
    if not os.path.exists(SUBMIT_DIR):
        local_tar = None
        for candidate in ("ascention.tgz", "expiation.tgz", "graysmoke_submission.tgz", "rail_aion_submission.tgz"):
...
def test_ascention_taskset_1(setup_public_area: int, setup_submit_area: int) -> None:
...
def test_ascention_taskset_2(setup_public_area: int, setup_submit_area: int) -> None:
    assert setup_public_area == 0
    assert setup_submit_area == 0
    run_taskset_2(
        PUBLIC_AREA,
...
def test_ascention_taskset_3(setup_public_area: int, setup_submit_area: int) -> None:
    assert setup_public_area == 0
    assert setup_submit_area == 0
    run_taskset_3(
        PUBLIC_AREA,
...
def test_ascention_taskset_4(setup_public_area: int, setup_submit_area: int) -> None:
    assert setup_public_area == 0
    assert setup_submit_area == 0
    run_taskset_4(
        PUBLIC_AREA,
